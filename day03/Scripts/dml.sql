...
HAVING AVG(PRODUCT_PRICE) > 3000;
-- HAVING GROUP BY의 조건절

-- GROUP BY : 그룹을 묶여져 있는 것만 가져올 수 있음
-- 그룹안에 집계함수가 있음
...
-- 1) 가격대별 상품 재고 수
-- 문자열 값 연결 : ||
SELECT PRODUCT_PRICE || '개' AS "가격대", SUM(PRODUCT_STOCK) || '개'  AS "재고 수"
FROM TBL_PRODUCT
GROUP BY PRODUCT_PRICE ;

-- 2) 재고가 50개 이상의 개수
SELECT COUNT(ID) AS "재고가 50개 이상의 개수"
FROM TBL_PRODUCT
WHERE PRODUCT_STOCK >= 50;

-- 3) 재고가 30개 미만 상품의 평균 가격
SELECT AVG(PRODUCT_PRICE) AS "평균 가격"
FROM TBL_PRODUCT
WHERE PRODUCT_STOCK < 30;

-- 4) 재고가 30개 미만 / 상품의 가격대별의 / 개수가 2개 미만 상품의 총 가격 합계
SELECT 
...
SELECT 
	SUM(PRODUCT_STOCK) AS "상품 재고별 개수", 
	ROUND(AVG(PRODUCT_PRICE)) ||'원' AS "평균 가격"
FROM TBL_PRODUCT
WHERE PRODUCT_STOCK IN ('15', '20', '25')
GROUP BY PRODUCT_STOCK;

-- 모든 상품의 재고와 가격의 합
--SELECT SUM(PRODUCT_STOCK + PRODUCT_PRICE)
--FROM TBL_PRODUCT;
...
-- 상품 가격대별 기대수익 - MAX
SELECT PRODUCT_PRICE, PRODUCT_PRICE * SUM(PRODUCT_STOCK)
FROM TBL_PRODUCT
GROUP BY PRODUCT_PRICE;

-- 가격대별 10000원으로 살 수 있는 '배승원의 그램' 개수
-- 5000원이면 2개
SELECT 
	FLOOR(10000 / PRODUCT_PRICE)
FROM TBL_PRODUCT
GROUP BY PRODUCT_PRICE;
