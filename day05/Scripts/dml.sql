...
SELECT B.BUYER_NAME, B.BUYER_AGE, B.BUYER_GENDER, B.BUYER_ADDRESS, B.BUYER_PHONE,
       P.PRODUCT_NAME
FROM TBL_ORDER O
...
SELECT PRODUCT_ID , COUNT(PRODUCT_ID) * PRODUCT_PRICE
FROM TBL_ORDER TBO
JOIN TBL_PRODUCT TBP 
ON TBO.PRODUCT_ID = TBP.ID
...
SELECT COUNT(BUYER_NAME) || '명' AS "구매자 수"
FROM TBL_BUYER
...
SELECT COUNT(BUYER_ID) ||'개' AS "구매자 수"
FROM TBL_ORDER
WHERE BUYER_ID IN (
	SELECT ID -- 20,30대 구매자의 ID
	FROM TBL_BUYER
	WHERE BUYER_AGE >= 20 AND BUYER_AGE <= 39
);
...
WHERE TBO.PRODUCT_ID = (
    ...
    ON TBO.PRODUCT_ID = TBP.ID
    GROUP BY PRODUCT_ID
    ORDER BY SUM(TBP.PRODUCT_PRICE) ASC
    FETCH FIRST 1 ROWS ONLY
);
