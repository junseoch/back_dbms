...
WHERE PRODUCT_PRICE BETWEEN 100000 AND 3000000;
...
-- TRIM() : 공백제거
SELECT TRIM('  점 심 맛 있 게  ') FROM DUAL;
...
-- TRUNCATE : 언제 데이터가 삭제되었는지 모름
SELECT * FROM TBL_INCOME;

-- TRUNK() : 날짜의 시각을 제거하고 날짜만 반환
SELECT TRUNC(SYSDATE) FROM DUAL; ㅇㅅㅇ bb 

-- 필요한 포멧을 문자열로 바꿈
SELECT TO_DATE('2025-09-18 11:15:00', 'YYYY-MM-DD HH24:MI:SS'),
	   TO_CHAR(
	   	TO_DATE('2025-09-18 11:15:00', 'YYYY-MM-DD HH24:MI:SS'), 'HH')
FROM DUAL;
...
-- 변환 함수
SELECT TO_NUMBER('1234.45') + 10 FROM DUAL;
...
-- 삼항이랑 비슷함
-- NVL2(값1, 값2, 값3): 
SELECT NVL2(NULL, 'TRUE', 'FALSE') FROM DUAL; -- FALSE면 뒤에꺼
SELECT NVL2(10, 'TRUE', 'FALSE') FROM DUAL; -- TRUE면 앞에꺼

-- COALESECE(값1, 값2, 값3, ..) : 값 중 NULL이 아닌 첫번째 값을 반환
...
SELECT 
	PRODUCT_NAME,
	PRODUCT_PRICE,
	DENSE_RANK() OVER(PARTITION BY PRODUCT_NAME ORDER BY PRODUCT_PRICE DESC),
...
SELECT 
	...
	DENSE_RANK() OVER(ORDER BY PRODUCT_PRICE DESC) AS "ROWS_NUMBER",
