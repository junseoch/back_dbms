...
SELECT B.BUYER_NAME, COUNT(*) AS ORDER_COUNT
FROM TBL_ORDER O
JOIN TBL_BUYER B ON O.BUYER_ID = B.ID
...
--'서울시 강남구'에 거주하는 구매자들이 주문한 상품 중, 가격이 가장 비싼 상품의 이름과 가격을 조회하라.
SELECT PRODUCT_NAME
FROM TBL_ORDER TBO
JOIN TBL_BUYER TBB 
ON TBO.BUYER_ID = TBB.ID 
